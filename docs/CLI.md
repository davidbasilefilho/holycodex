...
The executable name is `holycodex`. Development invocation is Bun-only and
may be `bun run holycodex ...` while the package is assembled; the installed
entry point has the same command surface. `mise` selects the repository's Bun
and TypeScript toolchain. npm, pnpm, and yarn are not valid invocation routes.
...
| `holycodex workflow run <file.ts\|-> [args.json] [--task <objective>] [--json]` | Start a validated TypeScript workflow. `--task` is the workflow objective; an explicit file gets the deterministic `workflow:<basename>` default, while stdin requires `--task`. Project files require matching project trust. |
...
`warnings` is always an array. Details are sanitized and bounded. The CLI
validates both input and output envelopes at the boundary with ArkType.
...
input is a validation failure, not a prompt.
