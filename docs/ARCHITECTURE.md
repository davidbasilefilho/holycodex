...
The runtime host validates every boundary and enforces the active plan before dispatch. Codex App Server is currently the supported programmatic bridge, so each `agent()` call starts an independent thread with the catalog-selected model, reasoning effort, low verbosity, and service tier. App Server does not expose the parent linkage used by native Codex collaboration subagents. HolyCodex must not present these independent threads as task-visible subagents. Native visibility requires a Codex host callback that dispatches through the active task's collaboration tools; the workflow runtime cannot invoke those tools directly. Codex remains authoritative for sandboxing, approvals, tool restrictions, skills, plugins, trust, and project boundaries. Interactive approvals are forwarded only on surfaces that support them; non-interactive runs fail closed when fresh approval is required.
