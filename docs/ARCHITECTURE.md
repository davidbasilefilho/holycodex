...
The runtime host validates every boundary and enforces the active plan before dispatch. Codex App Server is the supported programmatic bridge, so each `agent()` call starts an independent thread with the catalog-selected model, reasoning effort, low verbosity, and service tier. HolyCodex does not present these independent threads as task-visible subagents. Codex remains authoritative for sandboxing, approvals, tool restrictions, skills, plugins, trust, and project boundaries. Interactive approvals are forwarded only on surfaces that support them; non-interactive runs fail closed when fresh approval is required. Workflow execution has no wall-clock deadline and remains bounded by explicit cancellation, call and concurrency quotas, fan-out, retry and loop limits, memory, stack, and script size.
