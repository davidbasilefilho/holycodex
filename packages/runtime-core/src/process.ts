...
export type ManagedProcessInput = {
  ...
  readonly finalResolutionMs?: number;
};
  ...
  return await new Promise<ManagedProcessResult>((resolve) => {
    ...
    const terminate = (): void => {
      ...
      finalResolutionTimeout = setTimeout(
        () => finish(child.exitCode, "Managed process did not emit close after termination."),
        input.finalResolutionMs ?? runtime.terminationGraceMs * 2,
      );
