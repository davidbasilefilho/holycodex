...
describe("ensureDaemonRunning", () => {
  it("#given explicit daemon CLI env #when resolving spawn target #then uses that path", () => {
    expect(resolveDaemonCliPath({ CODEX_LSP_DAEMON_CLI: "/tmp/holycodex-lsp-daemon-cli.js" })).toBe(
      "/tmp/holycodex-lsp-daemon-cli.js",
    );
  });

  it("#given blank daemon CLI env #when resolving spawn target #then falls back to bundled cli", () => {
    expect(resolveDaemonCliPath({ CODEX_LSP_DAEMON_CLI: "  " })).toMatch(/cli\.(?:js|ts)$/);
  });
