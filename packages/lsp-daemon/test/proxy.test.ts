...
describe("mcp stdio proxy", () => {
  it("#given initialize and tools/call #when proxied #then initialize is local and the tool goes to the daemon", async () => {
    ...
    if (initialized === undefined) throw new Error("Missing initialize response");
    expect((initialized["result"] as { serverInfo?: unknown }).serverInfo).toBeDefined();
  ...
  it("#given a malformed line #when proxied #then returns a parse error", async () => {
    ...
    if (response === undefined) throw new Error("Missing parse-error response");
    expect((response["error"] as { code: number }).code).toBe(-32700);
  });
  ...
  it("#given an idle proxy #when the next request arrives after ten minutes #then it still responds", async () => {
    ...
    try {
      ...
      const response = responses[0];
      expect(response).toBeDefined();
      if (response === undefined) throw new Error("Missing initialize response");
