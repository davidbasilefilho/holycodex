...
export const LSP_INSTALL_HINTS: Readonly<Record<string, string>> = Object.freeze({
  ...
  razor: "dotnet tool install -g roslyn-language-server --prerelease",
});

const definitions: Readonly<Record<string, Omit<LspServerConfig, "id">>> = {
  typescript: {
  ...
  razor: { command: ["roslyn-language-server", "--stdio"], extensions: [".razor", ".cshtml"] },
};

export const BUILTIN_SERVERS: Readonly<Record<string, Omit<LspServerConfig, "id">>> = Object.freeze(
  Object.fromEntries(
    Object.entries(definitions).map(([id, definition]) => [id, Object.freeze(definition)]),
  ),
);

/** Returns the immutable builtin server definitions used for clean-room detection. */
export function builtinServerConfigs(): readonly LspServerConfig[] {
  return Object.entries(BUILTIN_SERVERS).map(([id, server]) => ({ id, ...server }));
}
