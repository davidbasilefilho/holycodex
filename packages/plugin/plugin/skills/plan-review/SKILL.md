...
1. Compare plan against request, repo, constraints, evidence, decisions. Map every material requirement to a step or exclusion; expose contradictions, unsupported assumptions, unresolved product choices. For qualifying UI or frontend implementation, require routing to the installed Build Web Apps `frontend-app-builder` skill and completion of its concept and design-approval workflow before implementation details; if absent, block and return it to `plan`. Read-only UI audits are exempt.
