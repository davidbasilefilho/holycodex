...
1. Compare plan against request, repo, constraints, evidence, decisions. Map every material requirement to a step or exclusion; expose contradictions, unsupported assumptions, unresolved product choices. For qualifying UI or frontend implementation, when Build Web Apps is available, require routing to `frontend-app-builder` and its concept and design-approval workflow before implementation details. If unavailable, tell user to enable Build Web Apps through Codex; do not block review solely on absence. Read-only UI audits are exempt.
