...
2. Select visible direction, type, layout/density, color/surfaces, assets, responsiveness, and states, plus a motion system and accessibility treatment for every task.
...
- AIDA, bento, hero geometry, counts, motion, and spacing are options only when they fit product, task, and stack.
- Use authentic supplied assets; ask before remote assets. Never invent research, content, proof, controls, or capabilities.
- Preserve project type unless redesign. Source-conditional, never universal, cautions: Fraunces + Work Sans as overused pairing; Space Grotesk + Inter as default tech; Cormorant Garamond/Bodoni Moda/Didot/Playfair as luxury autopilot; Sora as AI/deep-tech default; JetBrains Mono as fake-code/house voice; Syne as edgy default; Archivo as sport/streetwear default; Inter as identity/everywhere, though valid neutral body. Identity shortcuts: Inter, Space Grotesk, Sora, Syne, Archivo, Onest, Darker Grotesk, Geologica, Hanken Grotesk, Spline Sans, Schibsted Grotesk, Gabarito, Figtree, Quicksand; novelty display: Bagel Fat One, Baloo, Fredoka, Chewy, Lobster; identity serifs: Fraunces, Cormorant, Bodoni, Petrona, Hedvig Letters Serif, Brygada 1918, Young Serif; house monos: JetBrains Mono, IBM Plex Mono, Spline Sans Mono, Fragment Mono, except real data/code/timestamps/prices/tables; reputation swaps: Big Shoulders, Newsreader, IBM Plex Mono, Instrument Serif, Bricolage; startup defaults: Clash Display, General Sans. When triggered, choose viewed, authentic, licensed, self-hosted, project-owned, system, or distinctive type.
- New icons: never introduce Lucide. Preserve a non-Lucide repository system when it matches the accepted design; otherwise use Tabler Icons when its metaphor, fill/stroke, geometry, and weight match, then create a production-quality bare SVG when it does not. Accepted-design fidelity outranks library convenience. Use authorized brand marks only; never use emoji icons or default icon containers.
- Define purposeful motion. Use GSAP when installed or installation is permitted; otherwise use the existing stack or no dependency. Ask before installing. Keep content visible and meaning complete under `prefers-reduced-motion`; favor `transform`, `opacity`, `filter`; animate only state, continuity, navigation, or explanation.
